...
let ticketImg = document.getElementById('ticketImg');
...
let addTicketBtn = document.querySelector('.add-ticket-btn');
let removeTicketBtn = document.querySelector('.remove-ticket-btn');
let searchResultText = document.querySelector('.searchResult-text');
//
let searchBtn = document.querySelector('.search-btn');
let areaSelect = document.querySelector('.region-search');
let priceSelect = document.querySelector('.price-search');
let keywordInput = document.querySelector('.keyword-search');
...
let areaSearch = (e) => {
    let area = areaSelect.value.trim();
    ticketData = JSON.parse(localStorage.getItem('ticketData')) || remoteData;
    // 要給 C3.js 的資料
...
areaSelect.addEventListener('change', areaSearch, false);
...
// 上傳圖片練習
//參考: https://www.itread01.com/articles/1490385439.html 
...
uploadBtn.addEventListener('click', function(e){
    ...
    axios.post(url, formData, {
        headers: {
            // 用 Client-ID: 會變成隱藏圖片，Imgur 帳號找不到
            // Authorization: 'Client-ID c080331c5bf39db',
            // 用 token: Imgur 帳號可以找到
            Authorization: 'Bearer aa02ef6191c3a166426371b17fdcbff63fd684c8',
        }
    })
    .then(function (response) {
        console.log(response.data.data.link);
        ticketImg.value = response.data.data.link;
        tempImg.setAttribute('src', response.data.data.link);
    })
    .catch(function (error) {
        console.log(error);
    })
